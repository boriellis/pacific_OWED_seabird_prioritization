---
format: html
toc: TRUE
toc-title: "Table of Contents"
---

# **Rare California Seabird Distribution Expert Elicitation Exercise**

Thank you for helping us with this work! We hope that this exercise should take you less than an hour to complete. **Please read the instructions below carefully before filling out your responses in the Qualtrics survey linked below.**

There are multiple components of vulnerability to offshore wind energy for seabird species. We have existing tools that help us identify where birds are (e.g., SDMs such as [Leirness et al., 2021](https://repository.library.noaa.gov/view/noaa/49073){target="_blank"}), how sensitive birds may be to collision or displacement impacts (e.g., vulnerability indices such as [Kelsey et al., 2018](https://www.sciencedirect.com/science/article/abs/pii/S0301479718309228){target="_blank"}), and ways of describing bird population status (e.g., IUCN Red List status, state and federal listings, etc). **We are developing a tool to combine these existing understandings of the components of vulnerability to wind energy development to rank species vulnerability based on the placement of lease areas in any given region developing wind energy. The result will aid regional scale prioritization of mitigation and research efforts.**

## Goal of Expert Elicitation Exercise

**We need your help estimating the distributions of three threatened species: Short-tailed Albatross, Hawaiian Petrel, and Townsend's Storm Petrel. These species were not modeled by [Leirness et al., 2021](https://repository.library.noaa.gov/view/noaa/49073){target="_blank"} due to limited numbers of observations in the POCS region - we are trying to use expert elicitation to produce estimates of their distributions so that we may include them in the prioritization tool.**

The species distribution models (SDMs) developed by Leirness et al. (2021) describe the seasonal distributions of seabirds in the POCS region. In the maps below, you will find the model predictions for each species aggregated across seasons (i.e., one annual map per species). **We are asking you to use your experience as a seabird biologist and your knowledge of the life histories and habitat usage of seabirds to identify which of the existing SDMs you would expect to be most similar to the distributions of the three unknown species.** We will use your responses to estimate spatial overlap of the rare species (Short-tailed Albatross, Hawaiian Petrel, and Townsend’s Storm Petrel) with proposed wind energy areas. Additionally, we will use the degree of (dis)agreement among experts to quantify the uncertainty in the overlap estimate, which we will report alongside our findings.

## Instructions

1.  Begin by reviewing a brief overview of background information on the distribution of each of the three species [here](species_dossiers.html){target="_blank"}.

2.  Please open the qualtrics survey [here](https://ucsantacruz.co1.qualtrics.com/jfe/form/SV_d5uCEVtPOjK0yea){target="_blank"}. You will use the species maps below on this page to inform the answers that you'll provide on the spreadsheet, so we recommend opening both pages side by side. The survey has three main pages, one for each of the species we don't have distribution estimates for. Use the maps below on this page to inform your answers. *TIP: Once you scroll to the maps a full table of contents will populate in the side bar - use this to quickly flip between maps to compare them.*

3.  Use the sliders next to each species (/species group) to indicate how similar you think that species' spatial distribution is to the rare species. Please note the survey will not allow you to proceed to the next rare species until your similarities sum to 100%.

4.  Finally, please provide your name, contact info, and affiliation before you submit the survey. We won't link your names to your responses in our final product, but will reach out to you to provide you with final results and ask if you'd like to be listed in the acknowledgements.

### Examples using fictional species:

##### Example 1:

*The Soft-plumaged Skua is critically endangered and you've never seen one at sea, but you estimate based on your understanding of its life history that when it is found in the POCS region its habitat needs would be most similar to its closest relatives in the region. You drag the slider in the qualtrics survey for South Polar Skua to 50%, the slider for Pomarine Jaeger to 25%, and the slider for the combined Parasitic and Long-tailed Jaeger model to 25% and leave the rest at 0%. **The distribution of the Soft-plumaged Skua = 0.5(SPSK) + 0.25(POJA) + 0.25(PAJA-LTJA).***

![](images/Screenshot%202025-03-12%20at%2014.54.52.png)

##### Example 2:

*Based on your observations of the Silvery-cheeked Diving Petrel on at-sea surveys, you think it has a relatively coastal distribution with some particularly concentrated distributions around the Columbia River mouth. You drag the slider in the qualtrics survey for the Marbled Murrelet model to 70% and the slider for the Double-crested Cormorant model to 30% and leave the rest at 0%. **The distribution of the Silvery-cheeked Diving Petrel = 0.7(MAMU) + 0.3(DCCO).***

![](images/Screenshot%202025-03-12%20at%2014.54.25.png)

### Important notes:

-   For simplicity, the maps provided below have combined any seasonal models that were produced for the species into an annual model (so they effectively represent the averaged annual distribution for each species/group) - we recognize that species distributions vary dramatically seasonally, but please do your best in providing your answers to try to provide your estimation of the averaged annual distribution for the unknown species.

-   We ask that you complete every page of the survey. It's okay if your confidence in your estimates vary between species - we're calculating uncertainty in our final estimates based on the range of the answers provided across experts, and so it will be important for the analysis to have the same number of estimates for each species. You're welcome to add notes qualifying your answers or your confidence in your estimates at the bottom of each page.

-   Due to data limitations or high proportions of unidentified observations (e.g., "loon sp.") for particular taxa, some of the original models were produced for species groups instead of individual species. You can read more about this process [here](https://espis.boem.gov/final%20reports/BOEM_2021-014.pdf){target="_blank"} if you're interested, but as you're providing your responses please feel free to select whichever models you think best represent the unknown species you're providing answers for regardless of if it was produced for an individual species or a species group.

## Species maps

Use the maps below to inform your responses to the [qualtrics survey](https://ucsantacruz.co1.qualtrics.com/jfe/form/SV_d5uCEVtPOjK0yea){target="_blank"}.

The maps below have been simplified to aid comparison. We increased contrast for high-utilization regions by normalizing density relative to the maximum predicted density for that species (or species group). As such, please interpret these maps as *relative* spatial distributions. For example, the narrow band of yellow and orange along the coast in the Common Murre map indicates this species has an on-shelf distribution, whereas the wider band of yellow in the South Polar Skua map reflects its more pelagic distribution. The colors should not be interpreted as a measure of *absolute* population density. E.g., a yellow grid cell in the Common Murre map does not represent the same number of individual birds as a yellow grid cell in the South Polar Skua map.

Note that in some cases distributions are highly concentrated in coastal regions and it makes the visualization of distribution challenging - we recommend zooming in on coastal regions if a map looks blank at a glance.

### Combined Scoter Model

![](images/SCOT_density_map_binned.png)

### Combined Phalarope Model

![](images/PHAL_density_map_binned.png)

### Combined Parasitic & Long-tailed Jaeger Model

![](images/PAJA-LTJA_density_map_binned.png)

### Pomarine Jaeger

![](images/POJA_density_map_binned.png)

### South Polar Skua

![](images/SPSK_density_map_binned.png)

### Rhinoceros Auklet

![](images/RHAU_density_map_binned.png)

### Tufted Puffin

![](images/TUPU_density_map_binned.png)

### Cassin's Auklet

![](images/CAAU_density_map_binned.png)

### Marbled Murrelet

![](images/MAMU_density_map_binned.png)

### Pigeon Guillemot

![](images/PIGU_density_map_binned.png)

### Common Murre

![](images/COMU_density_map_binned.png)

### Ancient Murrelet

![](images/ANMU_density_map_binned.png)

### Combined Scripp's, Guadalupe, & Craveri's Murrelet Model

![](images/SCMU-GUMU-CRMU_density_map_binned.png)

### Black-legged Kittiwake

![](images/BLKI_density_map_binned.png)

### Sabine's Gull

![](images/SAGU_density_map_binned.png)

### Bonaparte's Gull

![](images/BOGU_density_map_binned.png)

### Heermann's Gull

![](images/HEEG_density_map_binned.png)

### Combined Western & Glaucous-winged Gull Model

![](images/WEGU-WGWH-GWGU_density_map_binned.png)

### California Gull

![](images/CAGU_density_map_binned.png)

### Combined Herring & Iceland Gull Model

![](images/HERG-ICGU_density_map_binned.png)

### Caspian Tern

![](images/CATE_density_map_binned.png)

### Combined Common & Arctic Tern Model

![](images/COTE-ARTE_density_map_binned.png)

### Combined Royal & Elegant Tern Model

![](images/ROYT-ELTE_density_map_binned.png)

### Combined Western & Clark's Grebe Model

![](images/WEGR-CLGR_density_map_binned.png)

### Red-throated Loon

![](images/RTLO_density_map_binned.png)

### Common Loon

![](images/COLO_density_map_binned.png)

### Combined Loon Model (includes Pacific Loon)

![](images/LOON_density_map_binned.png)

### Laysan Albatross

![](images/LAAL_density_map_binned.png)

### Black-footed Albatross

![](images/BFAL_density_map_binned.png)

### Fork-tailed Storm-Petrel

![](images/FTSP_density_map_binned.png)

### Leach's Storm Petrel

![](images/LESP_density_map_binned.png)

### Ashy Storm-Petrel

![](images/ASSP_density_map_binned.png)

### Black Storm-Petrel

![](images/BLSP_density_map_binned.png)

### Northern Fulmar

![](images/NOFU_density_map_binned.png)

### Murphy's Petrel

![](images/MUPE_density_map_binned.png)

### Cook's Petrel

![](images/COPE_density_map_binned.png)

### Pink-footed Shearwater

![](images/PFSH_density_map_binned.png)

### Buller's Shearwater

![](images/BULS_density_map_binned.png)

### Combined Short-tailed, Sooty, & Flesh-footed Shearwater Model

![](images/STTS-SOSH-FFSH_density_map_binned.png)

### Black-vented Shearwater

![](images/BVSH_density_map_binned.png)

### Brandt's Cormorant

![](images/BRAC_density_map_binned.png)

### Pelagic Cormorant

![](images/PECO_density_map_binned.png)

### Double-crested Cormorant

![](images/DCCO_density_map_binned.png)

### Brown Pelican

![](images/BRPE_density_map_binned.png)
