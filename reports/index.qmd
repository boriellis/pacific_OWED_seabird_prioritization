...
Based on the size of the lease areas compared to the overall region (i.e., \~.159% of the POCS region modeled is within wind energy areas), these numbers are typically very small (i.e., the highest exposure value in the dataset is \~0.8% overlap with the development areas, for a Pink-Footed Shearwater). For simplicity of interpretation I rescale these numbers from 0 to 1, such that the Pink-footed Shearwater’s value becomes a 1 as the species with the highest exposure value.

Jeff produced these models for individual species:

And these models were produced as groups for xxx reasons:

For the species that were modeled in groups that are included in this analysis, I'm doing xxx. Here are the detailed notes on each:

-   SOSH - use SOSH-FFSH-STTS, this is effectively a SOSH model in all seasons with a bit more messiness in winter

-   POJA - use POJA - it’s in other group models but they’re worse (WHAT SEASONS?)

-   PAJA - using PAJA-LTJA (NOTES HERE ON WHY) note that there’s also a JAEG model but that number is swayed heavily by POJA. make some comments on the flip flopping of frequency between seasons?

-   LTJA - using PAJA-LTJA (NOTES HERE ON WHY) note that there’s also a JAEG model but that number is swayed heavily by POJA. make some comments on the flip flopping of frequency between seasons?

-   HERG - use HERG-ICGU. This is primarily a HERG model in all seasons. 

-   ICGU - use HERG-ICGU. This is primarily a HERG model in all seasons. 

-   ARTE - use COTE-ARTE. This is mostly an ARTE model. 

-   RNPH - use PHAL. RNPH and REPH flip flop in frequency between seasons but there’s always high #s of unidentified. 

-   REPH - use PHAL. RNPH and REPH flip flop in frequency between seasons but there’s always high #s of unidentified.

-   SCMU - use SCMU-GUMU-CRMU. Note that we don’t have good spatial data on these based on low \# obs in all seasons and cryptic ids

-   GUMU - use SCMU-GUMU-CRMU. Note that we don’t have good spatial data on these based on low \# obs in all seasons and cryptic ids

-   CRMU - use SCMU-GUMU-CRMU. Note that we don’t have good spatial data on these based on low \# obs in all seasons and cryptic ids.

-   GWGU - use GWGU-WEGU. Almost entirely a WEGU model in all seasons but lumped thanks to hybrids and cryptic ids. 

-   WEGU- use GWGU-WEGU. Almost entirely a WEGU model in all seasons but lumped thanks to hybrids and cryptic ids

-   ELTE - use ELTE/ROYT, mostly an ELTE model anyway

-   COLO - Using COLO entirely. The model is only for SPSU, so should mention that, but there aren’t many obs of COLO in winter and fall anyway, and the LOON proportion is lower than COLO.

-   PALO - use LOON. this is mostly a PALO model in all seasons. Worth noting that it will be somewhat pulled by all the other loons. 

-   YBLO - use LOON, but note that this species is really data poor - this isn’t a great guess at where they are, it’s more like using PALO as a proxy.

-   RTLO - use RTLO. note that RTLO is only for SPSU, and LOON has higher proportion in lease areas overall? Still, it’s pretty similar

-   DCCO - use DCCO, but note that this is just based on SPSU, and by excluding winter distributions we’re swaying this a little precautionary 

-   PECO -  use PECO, but note that this is just based on SPSU, and by excluding winter distributions we’re swaying this a little precautionary

-   BRCO - use BRCO, but note that this is just based on SPSU, and by excluding winter distributions we’re swaying this a little precautionary

-   WEGR - use WEGR-CLGR, this is basically a WEGR model

-   CLGR - use WEGR-CLGR, but note that this is almost entirely a WEGR model, and that using this number for CLGR is highly precautionary

-   SUSC - use SCOT

-   BLSC - use SCOT but note that this model mostly represents SUSC and WWSC
