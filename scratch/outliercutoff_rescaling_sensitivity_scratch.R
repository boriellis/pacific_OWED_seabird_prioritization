...
#=============================================================================
# APPENDIX D — iterations retained per species
#=============================================================================
...
# one row per species: model used, and iterations/draws retained
iterations_table <- raw_1000 %>%
  ...
  select(common_name, exposure_model, n_retained = n_boot) %>%
  arrange(exposure_model, common_name)

# guard: exactly one row per species
...
iterations_table <- iterations_table %>% 
  mutate(n_removed = 200 - n_retained)
